...
void QsnUDPclient::actionConnect(quint16 address)
{
    ...
    if (!connected) {
        emit eventLog((isLog?0:0+5), tr("Bind failed"), QString("udpclient"), tr("UDP Client"), 4);
        return;
    }
    emit eventLog((isLog?2:2+5), tr("Bind port %1").arg(sourceport), QString("udpclient"), tr("UDP Client"), 4);
    registration();
}
...
void QsnUDPclient::snBUSInput(QSNContainer container, QObject *sender)
{
    ...
    if (container.role > QSNContainer::message && container.role != QSNContainer::information) return;
    if (container.role == QSNContainer::information) {
        ...
        if (options.count() > 1) container.Data = QString("%1/%2").arg(objectName()).arg(options[1]).toUtf8();
    }
    ...
    //if (container.role == 1) info(7, QString(tr("Send msg %1 %2->%3").arg(container.Command).arg(container.Sender).arg(container.Address)), false);
...
void QsnUDPclient::actionDisconnect()
{
    ...
    clientSocket->close();
    registredTimeOut = 0;
    emit eventLog((isLog?2:2+5), tr("Close"), QString("udpclient"), tr("UDP Client"), 4);
}
...
void QsnUDPclient::timeout()
{
    if (!autoRepeatMode) {
        ...
        clientSocket->abort();
        clientSocket->close();
        emit eventLog((isLog?0:0+5), tr("Connection lost"), QString("udpclient"), tr("UDP Client"), 4);
        emit connectedChange(false);
        return;
    }
    eventLog((isLog?0:0+5), tr("Connection lost, reconnect"), QString("udpclient"), tr("UDP Client"), 4);
    registredTimeOut = 60;
}
...
void QsnUDPclient::registrationAnswer(QSNContainer *container)
{
    ...
    if (container->Signal == 3) {
        clientSocket->abort();
        clientSocket->close();
        emit eventLog((isLog?0:0+5), tr("Registration is denied"), QString("udpclient"), tr("UDP Client"), 4);
        registredTimeOut = 0;
        connectedChange(false);
...
void QsnUDPclient::readedBlock(QByteArray *block)
{
    ...
    QSNContainer container = newContainer();
    container = containerFromStream(&stream);
    if (container.role == QSNContainer::registration) {
        registrationAnswer(&container);
        return;
    }
    //if (container.role == 1) info(7, QString(tr("Read msg %1 %2->%3").arg(container.Command).arg(container.Sender).arg(container.Address)), false);
    emit snBUSOutput(container, this);
}
