...
QsnInterface::~QsnInterface()
{
    // Stop timers and disconnect before member destructors run.
    // waitTimer/timeTimer are member variables (not children), so they
    // are destroyed AFTER ~QsnInterface(). Without this, the pending
    // signal disconnect accesses the already-freed QsnInterface → crash.
    waitTimer.stop();
    timeTimer.stop();
    disconnect(&waitTimer, nullptr, this, nullptr);
    disconnect(&timeTimer, nullptr, this, nullptr);
}
...
void QsnInterface::snBUSInput(QSNContainer container, QObject *sender)
{
    ...
    if (container.role == QSNContainer::signal)
    {
        ...
        eventReciveSignal(containerToSignal(container));
        snBUSOutput(container, sender);
        eventDBG(container, sender, false);
        return;
    }
...
void QsnInterface::ServiceProcessing(QSNContainer container)
{
    ...
    {
    case 1:
        if (container.Signal == 0) moduleConnected();
        if (container.Signal == 1) moduleDisconnected();
    }
}
...
void QsnInterface::netActionSetAddress(QSNMessage *message)
{
    ...
    eventDBG(container, this, true);
    actionSetAddress(deviceAddress);
    eventInfo(QString(tr("Address changed from %1 to %2")).arg(oldAddress).arg(deviceAddress));
}
...
bool QsnInterface::netActionNewDevice(QSNMessage *message)
{
    ...
    eventReciveDeviceInfo(message->Sender, QSNRAWtoUInt16(&message->Data,0), QSNRAWtoUInt16(&message->Data,2));
    return true;
}
...
void QsnInterface::netActionAnswerAddress(QSNMessage *message)
{
    for (int i = tasks.count() - 1; i >= 0; i--)
        if (tasks[i].operations == taskDevice::address &&
            ...
            eventDevAddressAnswer(tasks[i].devAddress, tasks[i].devNewAddress);
            tasks.remove(i);
            return;
        }
}
...
void QsnInterface::netActionAnswerParametr(QSNMessage *message)
{
    if (message->Address == deviceAddress) {
        ...
        if (debugProtocolV1) {
            ...
            D[0] = D[0] & 63;
            eventParametrAnswer(message->Sender, channel, D);
        } else {
...
{
    iConnected++;
    if (iConnected == 1)  eventConnect();
    eventConected(true);
    eventDisconected(false);
    eventStateChange(QString(tr("Device")),QString(tr("Address")),QString("%1").arg(deviceAddress));
}

void QsnInterface::moduleDisconnected()
{
    if (iConnected > 0) iConnected --;
    if (iConnected == 0) eventDisconnect();
    eventConected(false);
    eventDisconected(true);
}
...
void QsnInterface::checkDevInfoTasks(quint16 devAddr, quint16 devType, quint16 memorySize)
{
    for (int i = tasks.count() - 1; i >= 0; i--) {
        if (tasks[i].operations == taskDevice::info &&
            ...
            tasks.remove(i);
            eventDevInfoAnswer(devAddr, devType, memorySize);
            return;
        }
        ...
        if (tasks[i].operations == taskDevice::allInfo) {
            ...
            tasks[i].attempts = 0;
            eventDevAllInfoAnswer(devAddr, devType, memorySize);
            return;
        }
    }
    eventDevInfoUnsolicited(devAddr, devType, memorySize);
}
...
void QsnInterface::checkMemoryReadTasks(QSNMessage *message, taskDevice *task)
{
    ...
    if (task->blockPointer * 6 >= task->memory.count())   {
        eventMemoryReadAnswer(task->devAddress, task->memory, task->version);
        removeTask(task);
        return;
    }
    task->attempts = attemptsOperations;
    eventProgressTask(task->blockPointer, task->memory.count() / 6);
    checkMemoryReadTasksSend(task);
}

void QsnInterface::checkMemoryReadTasksSend(QsnInterface::taskDevice *task)
{
    task->waitTime = waitingTimeMemoryOperations;
    QSNContainer container = newContainer();
    container.role = QSNContainer::message;
...
void QsnInterface::checkMemoryWriteTasks(QsnInterface::taskDevice *task)
{
    ...
    if (addr >= task->memory.count())   {
        removeTask(task);
        eventMemoryWriteAnswer(task->devAddress);
        return;
    }
    ...
    for (int i = tasks.count() - 1; i >= 0; i--)
    {
        ...
        if (tasks[i].waitTime == 0) {
            ...
            case taskDevice::write:
                eventMemoryTimeOut(tasks[i].devAddress , static_cast<quint8>(tasks[i].attempts));
            ...
            case taskDevice::info:
                eventDevInfoTimeOut(tasks[i].devAddress);
                tasks.remove(i);
                break;
            case taskDevice::address:
                eventDevAddressTimeOut(tasks[i].devAddress, tasks[i].devNewAddress);
                tasks.remove(i);
                break;
    ...
    for (int i = 0; i < memoryIOCount; i ++)
        if (static_cast<quint8>(memory[memoryEnd - i * 3]) > 0) {
            if (QSNRAWtoUInt16(&memory, memoryEnd - i * 3 - 2) == signal)
                eventNumInput(memory[memoryEnd - i * 3] - 1, container->Data);
        }
}
...
void QsnInterface::actionDevMemoryReadRequest(quint16 addr, quint16 blockCount)
{
    ...
    dmr.waitTime = waitingTimeMemoryOperations;
    dmr.attempts = attemptsOperations;
    tasks.append(dmr);

    eventProgressTask(0, blockCount);
...
void QsnInterface::actionDevMemoryWriteRequest(quint16 addr, QByteArray memory)
{
    ...
    eventProgressTask(0, memoryBlocks);
...
void QsnInterface::loadSettings(QSettings *settings)
{
    ...
    autoConnectEnable = settings->value(QLatin1String("Interface_AutoConnect"), autoConnectEnable).toBool();
    debugProtocolV1 = settings->value(QLatin1String("Interface_DebugProtocolV1"), debugProtocolV1).toBool();
}
...
void QsnInterface::setDebugLevel(quint8 level)
{
    debuglevel = level;
}

quint8 QsnInterface::getDebugLevel()
{
    return debuglevel;
}
...
void QsnInterface::eventLog(quint8 warningLevel, QString text, QString fname, QString title, quint8 logmask)
{  
    if (!(getDebugLevel() & logmask)) return;
    QSNContainer container = newContainer();
    container.role = QSNContainer::information;
    container.Command = warningLevel;
    container.Address = static_cast<quint16>(logmask);
    container.Sender = 0;
    container.info = text;
    container.Data = QString("%1/%2").arg(fname).arg(title).toUtf8();
    container.Signal = 0;
    snBUSOutput(container, this);
}
